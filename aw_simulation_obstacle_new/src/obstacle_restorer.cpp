    ...
    namespace simulation
    {
                ...
                for (int64_t east_idx = east_start_idx; east_idx < east_end_idx; ++east_idx)
                {
                    for (int64_t north_idx = north_start_idx; north_idx < north_end_idx; ++north_idx)
                    {
                        ...
                        for (const std::string &file : files)
                        {
                            ...
                            dynamic_obstacle_[state_list_.id()] = std::make_shared<aw_simulation_obstacle::DynamicObstacleStateList>(state_list_);
                                                ...
                                                const double robot_utm_yaw)
        {
                 ...
                 it++)
            {
                ...
                if (((pow(robot_utm_east - it->second.utm_east, 2) + pow(robot_utm_north - it->second.utm_north, 2)) < 1 && abs(remainder1 - remainder2) < 1) || ((pow(robot_utm_east - it->second.utm_east, 2) + pow(robot_utm_north - it->second.utm_north, 2)) < 2 && it->second.yaw < -99))
                {
                    if (dynamic_obstacle_[it->first]->dynamic_obstacle_state_size() > 2)
                    {
                        ...
                        dynamic_obstacle_id_doing_[it->first].total_size = (dynamic_obstacle_[it->first]->dynamic_obstacle_state_size() - 1);
